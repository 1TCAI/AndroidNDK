...
GLuint GLTexture::CreateGLTextureFromFile(AAssetManager *assetManager, const char *fileName) {
    m_texID = generateTexture(assetManager,fileName);
    return m_texID;
}

GLuint GLTexture::generateTexture(AAssetManager *assetManager, const char *fileName) {
...
GLuint GLTexture::createOpenGLTexture(GLImage *pImg) {
    if(pImg == NULL){
        return -1;
    }
